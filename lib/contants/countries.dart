final countries = [
  {
    'name': 'United States',
    'phoneCode': '1',
    'countryCode': 'US',
    'flag': '🇺🇸'
  },
  {
    'name': 'Afghanistan',
    'phoneCode': '93',
    'countryCode': 'AF',
    'flag': '🇦🇫'
  },
  {'name': 'Albania', 'phoneCode': '355', 'countryCode': 'AL', 'flag': '🇦🇱'},
  {'name': 'Algeria', 'phoneCode': '213', 'countryCode': 'DZ', 'flag': '🇩🇿'},
  {
    'name': 'American Samoa',
    'phoneCode': '1-684',
    'countryCode': 'AS',
    'flag': '🇦🇸'
  },
  {'name': 'Andorra', 'phoneCode': '376', 'countryCode': 'AD', 'flag': '🇦🇩'},
  {'name': 'Angola', 'phoneCode': '244', 'countryCode': 'AO', 'flag': '🇦🇴'},
  {
    'name': 'Antigua and Barbuda',
    'phoneCode': '1-268',
    'countryCode': 'AG',
    'flag': '🇦🇬'
  },
  {'name': 'Argentina', 'phoneCode': '54', 'countryCode': 'AR', 'flag': '🇦🇷'},
  {'name': 'Armenia', 'phoneCode': '374', 'countryCode': 'AM', 'flag': '🇦🇲'},
  {'name': 'Aruba', 'phoneCode': '297', 'countryCode': 'AW', 'flag': '🇦🇼'},
  {'name': 'Australia', 'phoneCode': '61', 'countryCode': 'AU', 'flag': '🇦🇺'},
  {'name': 'Austria', 'phoneCode': '43', 'countryCode': 'AT', 'flag': '🇦🇹'},
  {
    'name': 'Azerbaijan',
    'phoneCode': '994',
    'countryCode': 'AZ',
    'flag': '🇦🇿'
  },
  {
    'name': 'Bahamas',
    'phoneCode': '1-242',
    'countryCode': 'BS',
    'flag': '🇧🇸'
  },
  {
    'name': 'Bangladesh',
    'phoneCode': '880',
    'countryCode': 'BD',
    'flag': '🇧🇩'
  },
  {
    'name': 'Barbados',
    'phoneCode': '1-246',
    'countryCode': 'BB',
    'flag': '🇧🇧'
  },
  {'name': 'Belarus', 'phoneCode': '375', 'countryCode': 'BY', 'flag': '🇧🇾'},
  {'name': 'Belgium', 'phoneCode': '32', 'countryCode': 'BE', 'flag': '🇧🇪'},
  {'name': 'Belize', 'phoneCode': '501', 'countryCode': 'BZ', 'flag': '🇧🇿'},
  {'name': 'Benin', 'phoneCode': '229', 'countryCode': 'BJ', 'flag': '🇧🇯'},
  {
    'name': 'Bermuda',
    'phoneCode': '1-441',
    'countryCode': 'BM',
    'flag': '🇧🇲'
  },
  {'name': 'Bhutan', 'phoneCode': '975', 'countryCode': 'BT', 'flag': '🇧🇹'},
  {'name': 'Bolivia', 'phoneCode': '591', 'countryCode': 'BO', 'flag': '🇧🇴'},
  {
    'name': 'Bosnia and Herzegovina',
    'phoneCode': '387',
    'countryCode': 'BA',
    'flag': '🇧🇦'
  },
  {'name': 'Botswana', 'phoneCode': '267', 'countryCode': 'BW', 'flag': '🇧🇼'},
  {'name': 'Brazil', 'phoneCode': '55', 'countryCode': 'BR', 'flag': '🇧🇷'},
  {
    'name': 'British Virgin Islands',
    'phoneCode': '1-284',
    'countryCode': 'VG',
    'flag': '🇻🇬'
  },
  {'name': 'Brunei', 'phoneCode': '673', 'countryCode': 'BN', 'flag': '🇧🇳'},
  {'name': 'Bulgaria', 'phoneCode': '359', 'countryCode': 'BG', 'flag': '🇧🇬'},
  {
    'name': 'Burkina Faso',
    'phoneCode': '226',
    'countryCode': 'BF',
    'flag': '🇧🇫'
  },
  {'name': 'Cambodia', 'phoneCode': '855', 'countryCode': 'KH', 'flag': '🇰🇭'},
  {'name': 'Cameroon', 'phoneCode': '237', 'countryCode': 'CM', 'flag': '🇨🇲'},
  {'name': 'Canada', 'phoneCode': '1', 'countryCode': 'CA', 'flag': '🇨🇦'},
  {
    'name': 'Cape Verde',
    'phoneCode': '238',
    'countryCode': 'CV',
    'flag': '🇨🇻'
  },
  {
    'name': 'Cayman Islands',
    'phoneCode': '1-345',
    'countryCode': 'KY',
    'flag': '🇰🇾'
  },
  {
    'name': 'Central African Republic',
    'phoneCode': '236',
    'countryCode': 'CF',
    'flag': '🇨🇫'
  },
  {'name': 'Chile', 'phoneCode': '56', 'countryCode': 'CL', 'flag': '🇨🇱'},
  {'name': 'Colombia', 'phoneCode': '57', 'countryCode': 'CO', 'flag': '🇨🇴'},
  {'name': 'Comoros', 'phoneCode': '269', 'countryCode': 'KM', 'flag': '🇰🇲'},
  {
    'name': 'Cook Islands',
    'phoneCode': '682',
    'countryCode': 'CK',
    'flag': '🇨🇰'
  },
  {
    'name': 'Costa Rica',
    'phoneCode': '506',
    'countryCode': 'CR',
    'flag': '🇨🇷'
  },
  {'name': 'Croatia', 'phoneCode': '385', 'countryCode': 'HR', 'flag': '🇭🇷'},
  {'name': 'Curacao', 'phoneCode': '599', 'countryCode': 'CW', 'flag': '🇨🇼'},
  {'name': 'Cyprus', 'phoneCode': '357', 'countryCode': 'CY', 'flag': '🇨🇾'},
  {
    'name': 'Czech Republic',
    'phoneCode': '420',
    'countryCode': 'CZ',
    'flag': '🇨🇿'
  },
  {
    'name': 'Democratic Republic of the Congo',
    'phoneCode': '243',
    'countryCode': 'CD',
    'flag': '🇨🇩'
  },
  {'name': 'Denmark', 'phoneCode': '45', 'countryCode': 'DK', 'flag': '🇩🇰'},
  {'name': 'Djibouti', 'phoneCode': '253', 'countryCode': 'DJ', 'flag': '🇩🇯'},
  {
    'name': 'Dominica',
    'phoneCode': '1-767',
    'countryCode': 'DM',
    'flag': '🇩🇲'
  },
  {
    'name': 'Dominican Republic',
    'phoneCode': '1-809',
    'countryCode': 'DO',
    'flag': '🇩🇴'
  },
  {
    'name': 'Dominican Republic',
    'phoneCode': '1-829',
    'countryCode': 'DO',
    'flag': '🇩🇴'
  },
  {
    'name': 'Dominican Republic',
    'phoneCode': '1-849',
    'countryCode': 'DO',
    'flag': '🇩🇴'
  },
  {
    'name': 'East Timor',
    'phoneCode': '670',
    'countryCode': 'TL',
    'flag': '🇹🇱'
  },
  {'name': 'Ecuador', 'phoneCode': '593', 'countryCode': 'EC', 'flag': '🇪🇨'},
  {'name': 'Egypt', 'phoneCode': '20', 'countryCode': 'EG', 'flag': '🇪🇬'},
  {
    'name': 'El Salvador',
    'phoneCode': '503',
    'countryCode': 'SV',
    'flag': '🇸🇻'
  },
  {
    'name': 'Equatorial Guinea',
    'phoneCode': '240',
    'countryCode': 'GQ',
    'flag': '🇬🇶'
  },
  {'name': 'Ethiopia', 'phoneCode': '251', 'countryCode': 'ET', 'flag': '🇪🇹'},
  {
    'name': 'Falkland Islands',
    'phoneCode': '500',
    'countryCode': 'FK',
    'flag': '🇫🇰'
  },
  {
    'name': 'Faroe Islands',
    'phoneCode': '298',
    'countryCode': 'FO',
    'flag': '🇫🇴'
  },
  {'name': 'Fiji', 'phoneCode': '679', 'countryCode': 'FJ', 'flag': '🇫🇯'},
  {'name': 'Finland', 'phoneCode': '358', 'countryCode': 'FI', 'flag': '🇫🇮'},
  {'name': 'France', 'phoneCode': '33', 'countryCode': 'FR', 'flag': '🇫🇷'},
  {
    'name': 'French Guiana',
    'phoneCode': '594',
    'countryCode': 'GF',
    'flag': '🇬🇫'
  },
  {
    'name': 'French Polynesia',
    'phoneCode': '689',
    'countryCode': 'PF',
    'flag': '🇵🇫'
  },
  {'name': 'Gabon', 'phoneCode': '241', 'countryCode': 'GA', 'flag': '🇬🇦'},
  {'name': 'Georgia', 'phoneCode': '995', 'countryCode': 'GE', 'flag': '🇬🇪'},
  {'name': 'Germany', 'phoneCode': '49', 'countryCode': 'DE', 'flag': '🇩🇪'},
  {'name': 'Ghana', 'phoneCode': '233', 'countryCode': 'GH', 'flag': '🇬🇭'},
  {
    'name': 'Gibraltar',
    'phoneCode': '350',
    'countryCode': 'GI',
    'flag': '🇬🇮'
  },
  {'name': 'Greece', 'phoneCode': '30', 'countryCode': 'GR', 'flag': '🇬🇷'},
  {
    'name': 'Greenland',
    'phoneCode': '299',
    'countryCode': 'GL',
    'flag': '🇬🇱'
  },
  {
    'name': 'Grenada',
    'phoneCode': '1-473',
    'countryCode': 'GD',
    'flag': '🇬🇩'
  },
  {
    'name': 'Guadeloupe',
    'phoneCode': '590',
    'countryCode': 'GP',
    'flag': '🇬🇵'
  },
  {'name': 'Guam', 'phoneCode': '1-671', 'countryCode': 'GU', 'flag': '🇬🇺'},
  {
    'name': 'Guatemala',
    'phoneCode': '502',
    'countryCode': 'GT',
    'flag': '🇬🇹'
  },
  {'name': 'Guinea', 'phoneCode': '224', 'countryCode': 'GN', 'flag': '🇬🇳'},
  {
    'name': 'Guinea-Bissau',
    'phoneCode': '245',
    'countryCode': 'GW',
    'flag': '🇬🇼'
  },
  {'name': 'Guyana', 'phoneCode': '592', 'countryCode': 'GY', 'flag': '🇬🇾'},
  {'name': 'Haiti', 'phoneCode': '509', 'countryCode': 'HT', 'flag': '🇭🇹'},
  {'name': 'Honduras', 'phoneCode': '504', 'countryCode': 'HN', 'flag': '🇭🇳'},
  {
    'name': 'Hong Kong',
    'phoneCode': '852',
    'countryCode': 'HK',
    'flag': '🇭🇰'
  },
  {'name': 'Hungary', 'phoneCode': '36', 'countryCode': 'HU', 'flag': '🇭🇺'},
  {'name': 'Iceland', 'phoneCode': '354', 'countryCode': 'IS', 'flag': '🇮🇸'},
  {'name': 'India', 'phoneCode': '91', 'countryCode': 'IN', 'flag': '🇮🇳'},
  {'name': 'Indonesia', 'phoneCode': '62', 'countryCode': 'ID', 'flag': '🇮🇩'},
  {'name': 'Iran', 'phoneCode': '98', 'countryCode': 'IR', 'flag': '🇮🇷'},
  {'name': 'Iraq', 'phoneCode': '964', 'countryCode': 'IQ', 'flag': '🇮🇶'},
  {'name': 'Ireland', 'phoneCode': '353', 'countryCode': 'IE', 'flag': '🇮🇪'},
  {'name': 'Israel', 'phoneCode': '972', 'countryCode': 'IL', 'flag': '🇮🇱'},
  {'name': 'Italy', 'phoneCode': '39', 'countryCode': 'IT', 'flag': '🇮🇹'},
  {
    'name': 'Jamaica',
    'phoneCode': '1-876',
    'countryCode': 'JM',
    'flag': '🇯🇲'
  },
  {'name': 'Japan', 'phoneCode': '81', 'countryCode': 'JP', 'flag': '🇯🇵'},
  {'name': 'Jordan', 'phoneCode': '962', 'countryCode': 'JO', 'flag': '🇯🇴'},
  {'name': 'Kazakhstan', 'phoneCode': '7', 'countryCode': 'KZ', 'flag': '🇰🇿'},
  {'name': 'Kenya', 'phoneCode': '254', 'countryCode': 'KE', 'flag': '🇰🇪'},
  {'name': 'Kiribati', 'phoneCode': '686', 'countryCode': 'KI', 'flag': '🇰🇮'},
  {'name': 'Kosovo', 'phoneCode': '383', 'countryCode': 'XK', 'flag': '🇽🇰'},
  {'name': 'Kuwait', 'phoneCode': '965', 'countryCode': 'KW', 'flag': '🇰🇼'},
  {
    'name': 'Kyrgyzstan',
    'phoneCode': '996',
    'countryCode': 'KG',
    'flag': '🇰🇬'
  },
  {'name': 'Laos', 'phoneCode': '856', 'countryCode': 'LA', 'flag': '🇱🇦'},
  {'name': 'Latvia', 'phoneCode': '371', 'countryCode': 'LV', 'flag': '🇱🇻'},
  {'name': 'Lebanon', 'phoneCode': '961', 'countryCode': 'LB', 'flag': '🇱🇧'},
  {'name': 'Lesotho', 'phoneCode': '266', 'countryCode': 'LS', 'flag': '🇱🇸'},
  {'name': 'Liberia', 'phoneCode': '231', 'countryCode': 'LR', 'flag': '🇱🇷'},
  {'name': 'Libya', 'phoneCode': '218', 'countryCode': 'LY', 'flag': '🇱🇾'},
  {
    'name': 'Liechtenstein',
    'phoneCode': '423',
    'countryCode': 'LI',
    'flag': '🇱🇮'
  },
  {
    'name': 'Lithuania',
    'phoneCode': '370',
    'countryCode': 'LT',
    'flag': '🇱🇹'
  },
  {
    'name': 'Luxembourg',
    'phoneCode': '352',
    'countryCode': 'LU',
    'flag': '🇱🇺'
  },
  {'name': 'Macau', 'phoneCode': '853', 'countryCode': 'MO', 'flag': '🇲🇴'},
  {
    'name': 'Macedonia',
    'phoneCode': '389',
    'countryCode': 'MK',
    'flag': '🇲🇰'
  },
  {
    'name': 'Madagascar',
    'phoneCode': '261',
    'countryCode': 'MG',
    'flag': '🇲🇬'
  },
  {'name': 'Malawi', 'phoneCode': '265', 'countryCode': 'MW', 'flag': '🇲🇼'},
  {'name': 'Malaysia', 'phoneCode': '60', 'countryCode': 'MY', 'flag': '🇲🇾'},
  {'name': 'Maldives', 'phoneCode': '960', 'countryCode': 'MV', 'flag': '🇲🇻'},
  {'name': 'Mali', 'phoneCode': '223', 'countryCode': 'ML', 'flag': '🇲🇱'},
  {'name': 'Malta', 'phoneCode': '356', 'countryCode': 'MT', 'flag': '🇲🇹'},
  {
    'name': 'Marshall Islands',
    'phoneCode': '692',
    'countryCode': 'MH',
    'flag': '🇲🇭'
  },
  {
    'name': 'Martinique',
    'phoneCode': '596',
    'countryCode': 'MQ',
    'flag': '🇲🇶'
  },
  {
    'name': 'Mauritania',
    'phoneCode': '222',
    'countryCode': 'MR',
    'flag': '🇲🇷'
  },
  {
    'name': 'Mauritius',
    'phoneCode': '230',
    'countryCode': 'MU',
    'flag': '🇲🇺'
  },
  {'name': 'Mayotte', 'phoneCode': '262', 'countryCode': 'YT', 'flag': '🇾🇹'},
  {'name': 'Mexico', 'phoneCode': '52', 'countryCode': 'MX', 'flag': '🇲🇽'},
  {
    'name': 'Micronesia',
    'phoneCode': '691',
    'countryCode': 'FM',
    'flag': '🇫🇲'
  },
  {'name': 'Moldova', 'phoneCode': '373', 'countryCode': 'MD', 'flag': '🇲🇩'},
  {'name': 'Monaco', 'phoneCode': '377', 'countryCode': 'MC', 'flag': '🇲🇨'},
  {'name': 'Mongolia', 'phoneCode': '976', 'countryCode': 'MN', 'flag': '🇲🇳'},
  {
    'name': 'Montenegro',
    'phoneCode': '382',
    'countryCode': 'ME',
    'flag': '🇲🇪'
  },
  {
    'name': 'Montserrat',
    'phoneCode': '1-664',
    'countryCode': 'MS',
    'flag': '🇲🇸'
  },
  {'name': 'Morocco', 'phoneCode': '212', 'countryCode': 'MA', 'flag': '🇲🇦'},
  {
    'name': 'Mozambique',
    'phoneCode': '258',
    'countryCode': 'MZ',
    'flag': '🇲🇿'
  },
  {'name': 'Myanmar', 'phoneCode': '95', 'countryCode': 'MM', 'flag': '🇲🇲'},
  {'name': 'Namibia', 'phoneCode': '264', 'countryCode': 'NA', 'flag': '🇳🇦'},
  {'name': 'Nauru', 'phoneCode': '674', 'countryCode': 'NR', 'flag': '🇳🇷'},
  {'name': 'Nepal', 'phoneCode': '977', 'countryCode': 'NP', 'flag': '🇳🇵'},
  {
    'name': 'Netherlands',
    'phoneCode': '31',
    'countryCode': 'NL',
    'flag': '🇳🇱'
  },
  {
    'name': 'New Caledonia',
    'phoneCode': '687',
    'countryCode': 'NC',
    'flag': '🇳🇨'
  },
  {
    'name': 'New Zealand',
    'phoneCode': '64',
    'countryCode': 'NZ',
    'flag': '🇳🇿'
  },
  {
    'name': 'Nicaragua',
    'phoneCode': '505',
    'countryCode': 'NI',
    'flag': '🇳🇮'
  },
  {'name': 'Niger', 'phoneCode': '227', 'countryCode': 'NE', 'flag': '🇳🇪'},
  {'name': 'Nigeria', 'phoneCode': '234', 'countryCode': 'NG', 'flag': '🇳🇬'},
  {'name': 'Niue', 'phoneCode': '683', 'countryCode': 'NU', 'flag': '🇳🇺'},
  {
    'name': 'Norfolk Island',
    'phoneCode': '672',
    'countryCode': 'NF',
    'flag': '🇳🇫'
  },
  {
    'name': 'North Korea',
    'phoneCode': '850',
    'countryCode': 'KP',
    'flag': '🇰🇵'
  },
  {
    'name': 'Northern Mariana Islands',
    'phoneCode': '1-670',
    'countryCode': 'MP',
    'flag': '🇲🇵'
  },
  {'name': 'Norway', 'phoneCode': '47', 'countryCode': 'NO', 'flag': '🇳🇴'},
  {'name': 'Oman', 'phoneCode': '968', 'countryCode': 'OM', 'flag': '🇴🇲'},
  {'name': 'Pakistan', 'phoneCode': '92', 'countryCode': 'PK', 'flag': '🇵🇰'},
  {'name': 'Palau', 'phoneCode': '680', 'countryCode': 'PW', 'flag': '🇵🇼'},
  {
    'name': 'Palestine',
    'phoneCode': '970',
    'countryCode': 'PS',
    'flag': '🇵🇸'
  },
  {'name': 'Panama', 'phoneCode': '507', 'countryCode': 'PA', 'flag': '🇵🇦'},
  {
    'name': 'Papua New Guinea',
    'phoneCode': '675',
    'countryCode': 'PG',
    'flag': '🇵🇬'
  },
  {'name': 'Paraguay', 'phoneCode': '595', 'countryCode': 'PY', 'flag': '🇵🇾'},
  {'name': 'Peru', 'phoneCode': '51', 'countryCode': 'PE', 'flag': '🇵🇪'},
  {
    'name': 'Philippines',
    'phoneCode': '63',
    'countryCode': 'PH',
    'flag': '🇵🇭'
  },
  {'name': 'Poland', 'phoneCode': '48', 'countryCode': 'PL', 'flag': '🇵🇱'},
  {'name': 'Portugal', 'phoneCode': '351', 'countryCode': 'PT', 'flag': '🇵🇹'},
  {
    'name': 'Puerto Rico',
    'phoneCode': '1-787',
    'countryCode': 'PR',
    'flag': '🇵🇷'
  },
  {
    'name': 'Puerto Rico',
    'phoneCode': '1-939',
    'countryCode': 'PR',
    'flag': '🇵🇷'
  },
  {'name': 'Qatar', 'phoneCode': '974', 'countryCode': 'QA', 'flag': '🇶🇦'},
  {
    'name': 'Republic of the Congo',
    'phoneCode': '242',
    'countryCode': 'CG',
    'flag': '🇨🇬'
  },
  {'name': 'Reunion', 'phoneCode': '262', 'countryCode': 'RE', 'flag': '🇷🇪'},
  {'name': 'Romania', 'phoneCode': '40', 'countryCode': 'RO', 'flag': '🇷🇴'},
  {'name': 'Russia', 'phoneCode': '7', 'countryCode': 'RU', 'flag': '🇷🇺'},
  {'name': 'Rwanda', 'phoneCode': '250', 'countryCode': 'RW', 'flag': '🇷🇼'},
  {
    'name': 'Saint Barthelemy',
    'phoneCode': '590',
    'countryCode': 'BL',
    'flag': '🇧🇱'
  },
  {
    'name': 'Saint Helena',
    'phoneCode': '290',
    'countryCode': 'SH',
    'flag': '🇸🇭'
  },
  {
    'name': 'Saint Kitts and Nevis',
    'phoneCode': '1-869',
    'countryCode': 'KN',
    'flag': '🇰🇳'
  },
  {
    'name': 'Saint Lucia',
    'phoneCode': '1-758',
    'countryCode': 'LC',
    'flag': '🇱🇨'
  },
  {
    'name': 'Saint Martin',
    'phoneCode': '590',
    'countryCode': 'MF',
    'flag': '🇲🇫'
  },
  {
    'name': 'Saint Pierre and Miquelon',
    'phoneCode': '508',
    'countryCode': 'PM',
    'flag': '🇵🇲'
  },
  {
    'name': 'Saint Vincent and the Grenadines',
    'phoneCode': '1-784',
    'countryCode': 'VC',
    'flag': '🇻🇨'
  },
  {'name': 'Samoa', 'phoneCode': '685', 'countryCode': 'WS', 'flag': '🇼🇸'},
  {
    'name': 'San Marino',
    'phoneCode': '378',
    'countryCode': 'SM',
    'flag': '🇸🇲'
  },
  {
    'name': 'Sao Tome and Principe',
    'phoneCode': '239',
    'countryCode': 'ST',
    'flag': '🇸🇹'
  },
  {
    'name': 'Saudi Arabia',
    'phoneCode': '966',
    'countryCode': 'SA',
    'flag': '🇸🇦'
  },
  {'name': 'Senegal', 'phoneCode': '221', 'countryCode': 'SN', 'flag': '🇸🇳'},
  {'name': 'Serbia', 'phoneCode': '381', 'countryCode': 'RS', 'flag': '🇷🇸'},
  {
    'name': 'Seychelles',
    'phoneCode': '248',
    'countryCode': 'SC',
    'flag': '🇸🇨'
  },
  {
    'name': 'Sierra Leone',
    'phoneCode': '232',
    'countryCode': 'SL',
    'flag': '🇸🇱'
  },
  {'name': 'Singapore', 'phoneCode': '65', 'countryCode': 'SG', 'flag': '🇸🇬'},
  {
    'name': 'Sint Maarten',
    'phoneCode': '1-721',
    'countryCode': 'SX',
    'flag': '🇸🇽'
  },
  {'name': 'Slovakia', 'phoneCode': '421', 'countryCode': 'SK', 'flag': '🇸🇰'},
  {'name': 'Slovenia', 'phoneCode': '386', 'countryCode': 'SI', 'flag': '🇸🇮'},
  {
    'name': 'Solomon Islands',
    'phoneCode': '677',
    'countryCode': 'SB',
    'flag': '🇸🇧'
  },
  {'name': 'Somalia', 'phoneCode': '252', 'countryCode': 'SO', 'flag': '🇸🇴'},
  {
    'name': 'South Africa',
    'phoneCode': '27',
    'countryCode': 'ZA',
    'flag': '🇿🇦'
  },
  {
    'name': 'South Korea',
    'phoneCode': '82',
    'countryCode': 'KR',
    'flag': '🇰🇷'
  },
  {
    'name': 'South Sudan',
    'phoneCode': '211',
    'countryCode': 'SS',
    'flag': '🇸🇸'
  },
  {'name': 'Spain', 'phoneCode': '34', 'countryCode': 'ES', 'flag': '🇪🇸'},
  {'name': 'Sri Lanka', 'phoneCode': '94', 'countryCode': 'LK', 'flag': '🇱🇰'},
  {'name': 'Sudan', 'phoneCode': '249', 'countryCode': 'SD', 'flag': '🇸🇩'},
  {'name': 'Suriname', 'phoneCode': '597', 'countryCode': 'SR', 'flag': '🇸🇷'},
  {
    'name': 'Swaziland',
    'phoneCode': '268',
    'countryCode': 'SZ',
    'flag': '🇸🇿'
  },
  {'name': 'Sweden', 'phoneCode': '46', 'countryCode': 'SE', 'flag': '🇸🇪'},
  {
    'name': 'Switzerland',
    'phoneCode': '41',
    'countryCode': 'CH',
    'flag': '🇨🇭'
  },
  {'name': 'Syria', 'phoneCode': '963', 'countryCode': 'SY', 'flag': '🇸🇾'},
  {'name': 'Taiwan', 'phoneCode': '886', 'countryCode': 'TW', 'flag': '🇹🇼'},
  {
    'name': 'Tajikistan',
    'phoneCode': '992',
    'countryCode': 'TJ',
    'flag': '🇹🇯'
  },
  {'name': 'Tanzania', 'phoneCode': '255', 'countryCode': 'TZ', 'flag': '🇹🇿'},
  {'name': 'Thailand', 'phoneCode': '66', 'countryCode': 'TH', 'flag': '🇹🇭'},
  {
    'name': 'Timor-Leste',
    'phoneCode': '670',
    'countryCode': 'TL',
    'flag': '🇹🇱'
  },
  {'name': 'Togo', 'phoneCode': '228', 'countryCode': 'TG', 'flag': '🇹🇬'},
  {'name': 'Tokelau', 'phoneCode': '690', 'countryCode': 'TK', 'flag': '🇹🇰'},
  {'name': 'Tonga', 'phoneCode': '676', 'countryCode': 'TO', 'flag': '🇹🇴'},
  {
    'name': 'Trinidad and Tobago',
    'phoneCode': '1-868',
    'countryCode': 'TT',
    'flag': '🇹🇹'
  },
  {'name': 'Tunisia', 'phoneCode': '216', 'countryCode': 'TN', 'flag': '🇹🇳'},
  {'name': 'Turkey', 'phoneCode': '90', 'countryCode': 'TR', 'flag': '🇹🇷'},
  {
    'name': 'Turkmenistan',
    'phoneCode': '993',
    'countryCode': 'TM',
    'flag': '🇹🇲'
  },
  {'name': 'Tuvalu', 'phoneCode': '688', 'countryCode': 'TV', 'flag': '🇹🇻'},
  {
    'name': 'U.S. Virgin Islands',
    'phoneCode': '1-340',
    'countryCode': 'VI',
    'flag': '🇻🇮'
  },
  {'name': 'Uganda', 'phoneCode': '256', 'countryCode': 'UG', 'flag': '🇺🇬'},
  {'name': 'Ukraine', 'phoneCode': '380', 'countryCode': 'UA', 'flag': '🇺🇦'},
  {
    'name': 'United Arab Emirates',
    'phoneCode': '971',
    'countryCode': 'AE',
    'flag': '🇦🇪'
  },
  {
    'name': 'United Kingdom',
    'phoneCode': '44',
    'countryCode': 'GB',
    'flag': '🇬🇧'
  },
  {'name': 'Uruguay', 'phoneCode': '598', 'countryCode': 'UY', 'flag': '🇺🇾'},
  {
    'name': 'Uzbekistan',
    'phoneCode': '998',
    'countryCode': 'UZ',
    'flag': '🇺🇿'
  },
  {'name': 'Vanuatu', 'phoneCode': '678', 'countryCode': 'VU', 'flag': '🇻🇺'},
  {'name': 'Vatican', 'phoneCode': '379', 'countryCode': 'VA', 'flag': '🇻🇦'},
  {'name': 'Venezuela', 'phoneCode': '58', 'countryCode': 'VE', 'flag': '🇻🇪'},
  {'name': 'Vietnam', 'phoneCode': '84', 'countryCode': 'VN', 'flag': '🇻🇳'},
  {
    'name': 'Wallis and Futuna',
    'phoneCode': '681',
    'countryCode': 'WF',
    'flag': '🇼🇫'
  },
  {
    'name': 'Western Sahara',
    'phoneCode': '212',
    'countryCode': 'EH',
    'flag': '🇪🇭'
  },
  {'name': 'Yemen', 'phoneCode': '967', 'countryCode': 'YE', 'flag': '🇾🇪'},
  {'name': 'Zambia', 'phoneCode': '260', 'countryCode': 'ZM', 'flag': '🇿🇲'},
  {'name': 'Zimbabwe', 'phoneCode': '263', 'countryCode': 'ZW', 'flag': '🇿🇼'}
];
